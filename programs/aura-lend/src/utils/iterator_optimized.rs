...
pub mod optimized_iterators {
    ...
    pub fn calculate_total_borrowed_value_optimized(
        ...
        for borrow in borrows.iter()
            ...
            .filter(|b| !b.market_value_usd.is_zero()) // Skip zero market values
        {
    ...
    pub fn calculate_weighted_ltv_optimized(
        deposits: &[ObligationCollateral]
    ) -> Result<u64> {
        ...
        for deposit in deposits.iter()
            ...
            .take(16) // Limit to prevent excessive computation
        {
            ...
            weighted_ltv = weighted_ltv.saturating_add(
                value_u128.saturating_mul(deposit.loan_to_value_bps as u128)
            );
        }
...
pub mod performance_bench {
    ...
    pub fn benchmark_lookup_operations(iterations: usize) -> (u128, u128) {
        ...
        for _ in 0..iterations {
            for (deposits, borrows) in &test_data {
                let mut calculator = optimized_iterators::HealthFactorCalculator::new(deposits, borrows);
    ...
    fn generate_test_obligations(count: usize) -> Vec<(Vec<ObligationCollateral>, Vec<ObligationLiquidity>)> {
