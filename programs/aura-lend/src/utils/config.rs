...
impl ProtocolConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        require!(
            self.max_reserves > 0 && self.max_reserves <= 1000,
            LendingError::InvalidConfiguration
        );
        require!(
            self.max_obligations > 0 && self.max_obligations <= 100_000,
            LendingError::InvalidConfiguration
        );
        require!(
            self.max_obligation_reserves > 0 && self.max_obligation_reserves <= 32,
            LendingError::InvalidConfiguration
        );

        // Economic parameters validation
        require!(
            self.default_protocol_fee_bps <= BASIS_POINTS_PRECISION,
            LendingError::InvalidConfiguration
        );
        require!(
            self.max_protocol_fee_bps <= BASIS_POINTS_PRECISION,
            LendingError::InvalidConfiguration
        );
        require!(
            self.liquidation_close_factor_bps > 0
                && self.liquidation_close_factor_bps <= BASIS_POINTS_PRECISION,
            LendingError::InvalidConfiguration
        );
        require!(
        ...
        require!(
            self.min_health_factor >= PRECISION,
            LendingError::InvalidConfiguration
        ); // At least 1.0
        require!(
            self.max_ltv_ratio > 0 && self.max_ltv_ratio <= 9000,
            LendingError::InvalidConfiguration
        ); // Max 90%
        require!(
        ...
        require!(
            self.max_oracle_staleness_slots > 0 && self.max_oracle_staleness_slots <= 14400,
            LendingError::InvalidConfiguration
        ); // Max 2 hours
        require!(
            self.max_oracle_confidence_threshold <= 10000,
            LendingError::InvalidConfiguration
        ); // Max 100%
        require!(
            self.min_oracle_sources > 0 && self.min_oracle_sources <= 10,
            LendingError::InvalidConfiguration
        );

        // Governance settings validation
        require!(
            self.max_multisig_signatories >= 2 && self.max_multisig_signatories <= 50,
            LendingError::InvalidConfiguration
        );
        require!(
            self.min_multisig_threshold >= 1
                && self.min_multisig_threshold <= self.max_multisig_signatories,
            LendingError::InvalidConfiguration
        );
        require!(
            self.max_governance_roles > 0 && self.max_governance_roles <= 1000,
            LendingError::InvalidConfiguration
        );
        require!(
            self.default_timelock_delay >= 3600,
            LendingError::InvalidConfiguration
        ); // Min 1 hour

        // Performance settings validation
        require!(
            self.compute_unit_limit >= 200_000 && self.compute_unit_limit <= 1_400_000,
            LendingError::InvalidConfiguration
        );
        require!(
            self.max_accounts_per_instruction > 0 && self.max_accounts_per_instruction <= 64,
            LendingError::InvalidConfiguration
        );
        require!(
            self.pagination_default_limit > 0
                && self.pagination_default_limit <= self.pagination_max_limit,
            LendingError::InvalidConfiguration
        );
        require!(
            self.pagination_max_limit > 0 && self.pagination_max_limit <= 1000,
            LendingError::InvalidConfiguration
        );
    ...
    pub fn get_protocol_fee_bps(&self, reserve_fee_bps: Option<u64>) -> u64 {
        reserve_fee_bps
            .unwrap_or(self.default_protocol_fee_bps)
            .min(self.max_protocol_fee_bps)
    }
...
impl ConfigUpdateParams {
    ...
    pub fn apply_to(&self, config: &mut ProtocolConfig) {
        // Market limits
        if let Some(value) = self.max_reserves {
            config.max_reserves = value;
        }
        if let Some(value) = self.max_obligations {
            config.max_obligations = value;
        }
        if let Some(value) = self.max_obligation_reserves {
            config.max_obligation_reserves = value;
        }

        // Economic parameters
        if let Some(value) = self.default_protocol_fee_bps {
            config.default_protocol_fee_bps = value;
        }
        if let Some(value) = self.max_protocol_fee_bps {
            config.max_protocol_fee_bps = value;
        }
        if let Some(value) = self.liquidation_close_factor_bps {
            config.liquidation_close_factor_bps = value;
        }
        ...
        if let Some(value) = self.min_health_factor {
            config.min_health_factor = value;
        }
        if let Some(value) = self.max_ltv_ratio {
            config.max_ltv_ratio = value;
        }
        ...
        if let Some(value) = self.max_oracle_staleness_slots {
            config.max_oracle_staleness_slots = value;
        }
        if let Some(value) = self.max_oracle_confidence_threshold {
            config.max_oracle_confidence_threshold = value;
        }
        if let Some(value) = self.min_oracle_sources {
            config.min_oracle_sources = value;
        }

        // Governance settings
        if let Some(value) = self.max_multisig_signatories {
            config.max_multisig_signatories = value;
        }
        if let Some(value) = self.min_multisig_threshold {
            config.min_multisig_threshold = value;
        }
        if let Some(value) = self.max_governance_roles {
            config.max_governance_roles = value;
        }
        if let Some(value) = self.default_timelock_delay {
            config.default_timelock_delay = value;
        }

        // Performance settings
        if let Some(value) = self.compute_unit_limit {
            config.compute_unit_limit = value;
        }
        if let Some(value) = self.max_accounts_per_instruction {
            config.max_accounts_per_instruction = value;
        }
        if let Some(value) = self.pagination_default_limit {
            config.pagination_default_limit = value;
        }
        if let Some(value) = self.pagination_max_limit {
            config.pagination_max_limit = value;
        }

        // Emergency settings
        if let Some(value) = self.emergency_mode {
            config.emergency_mode = value;
        }
        if let Some(value) = self.pause_deposits {
            config.pause_deposits = value;
        }
        if let Some(value) = self.pause_withdrawals {
            config.pause_withdrawals = value;
        }
        if let Some(value) = self.pause_borrows {
            config.pause_borrows = value;
        }
